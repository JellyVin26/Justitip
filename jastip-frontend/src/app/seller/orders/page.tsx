  ...
  return (
    <div className="min-h-screen bg-navy-950 text-zinc-100">
      <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
        <h1 className="mb-2 text-3xl font-extrabold tracking-tight">Orders</h1>
        <p className="mb-6 text-sm text-zinc-400">Manage requests on your trips and listings.</p>
        ...
        {filteredOrders.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-navy-900 p-12 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-zinc-500">
              Once buyers start placing orders on your listings, they will appear here for you to manage.
            </p>
          </div>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-navy-900">
            ...
            <div className="divide-y divide-white/5 md:hidden">
              {filteredOrders.map((order) => (
                <Link key={order.id} href={`/seller/orders/${order.id}`} className="flex items-center gap-4 p-4 transition-colors hover:bg-white/5">
                  <img
                    src={order.productImageUrl || "https://picsum.photos/seed/justitip-order/100/100"}
                    alt={order.productName}
